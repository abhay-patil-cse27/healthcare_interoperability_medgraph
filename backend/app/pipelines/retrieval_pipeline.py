...
================================
Hybrid RAG pipeline: Neo4j graph search + OpenSearch vector search
...
class RetrievalPipeline:
    ...
    def _build_graph(self):
        ...
        g.add_node("graph_search", self._graph_search)
        ...
        g.add_edge("generate_query_embedding", "graph_search")
        g.add_edge("graph_search", "hybrid_rank")
    ...
    async def run(
        ...
        return {
            ...
            "guardrail_action": final.get("guardrail_action", "NONE"),
        }
    ...
    async def _graph_search(self, state: RetrievalState) -> dict:
        """Parallel search: Neo4j graph + OpenSearch vector."""
        import asyncio
        ...
        vector_task = self.vector_store.search(
            ...
            scope=state["consent_scope"],
            filters=state["consent_filters"],
    ...
    async def _invoke_llm(self, state: RetrievalState) -> dict:
        result = await self.llm.invoke(
            ...
            apply_guardrails=False,  # Disabled: data is consent-gated + PHI-redacted
        )
        return {
            **state,
